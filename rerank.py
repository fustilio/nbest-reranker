...
def rerank(input_nbest, weights, out_dir, clean_up):
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)


    output_nbest_path = out_dir + '/augmented.nbest'
    shutil.copy(input_nbest, output_nbest_path)

    with open(weights, 'r') as input_weights:
    ...
    prefix = os.path.basename(input_nbest)
    input_aug_nbest = NBestList(output_nbest_path, mode='r')
    output_nbest = NBestList(out_dir + '/' + prefix + '.reranked.nbest', mode='w')
    output_1best = codecs.open(out_dir + '/' + prefix + '.reranked.1best', mode='w', encoding='UTF-8')
...
if __name__ == '__main__':
    ...
    L.set_logger(os.path.abspath(out_dir),'train_log.txt')
    L.print_args(args)

    rerank(args.input_nbest, weights, out_dir, clean_up)
